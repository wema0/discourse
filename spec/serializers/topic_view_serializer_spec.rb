...
describe TopicViewSerializer do
  ...
  context "with flags" do
    ...
    it "will return reviewable counts on posts" do
      ...
      expect(p0[:reviewable_count]).to eq(2)
      expect(p0[:reviewable_pending_count]).to eq(1)

      p1 = json[:post_stream][:posts][1]
      expect(p1[:reviewable_count]).to eq(0)
      expect(p1[:reviewable_pending_count]).to eq(0)
    end
  end
